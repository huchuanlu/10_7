...
objectHistogramPyramid = calculatePyramidFeatures_IH(hogIH, patchParam, hogBins);
...
for num = trainFrameNumber+1:endFrameNumber  
    ...
    for rr = searchWindowT(1):searchWindowT(2)
        for cc = searchWindowT(3):searchWindowT(4)
            ...
            candidateHistogramPyramid = calculatePyramidFeatures_IH(hogIH, patchParam, hogBins);
            confidenceHoG(rr-searchWindowT(1)+1,cc-searchWindowT(3)+1) = calculatePyramidMatching(objectHistogramPyramid, candidateHistogramPyramid)/maxMatchNum;
        end
    end
    
    confidenceHoG = exp(sigmaHoG*confidenceHoG);
    confidenceHoG = confidenceHoG./max(max(confidenceHoG));

    temp = zeros(size(confidenceHoG));
    for rr = 1+halfMW:size(temp,1)-halfMW
        for cc = 1+halfMW:size(temp,2)-halfMW
            temp(rr,cc) = sum(sum(confidenceHoG(rr-halfsz:rr+halfsz,cc-halfsz:cc+halfsz)));
        end
    end
