...
objectHistogramPyramid = calculatePyramidFeatures_IH(grayIH, patchParam, grayBins);
...
for num = trainFrameNumber+1:endFrameNumber  
    ...
    for rr = searchWindowT(1):searchWindowT(2)
        for cc = searchWindowT(3):searchWindowT(4)
            ...
            candidateHistogramPyramid = calculatePyramidFeatures_IH(grayIH, patchParam, grayBins);
            confidenceGray(rr-searchWindowT(1)+1,cc-searchWindowT(3)+1) = calculatePyramidMatching(objectHistogramPyramid, candidateHistogramPyramid)/maxMatchNum;
        end
    end
    confidenceGray = exp(sigmaGray*confidenceGray);
    confidenceGray = confidenceGray./max(max(confidenceGray));

    temp = zeros(size(confidenceGray));
    for rr = 1+halfMW:size(temp,1)-halfMW
        for cc = 1+halfMW:size(temp,2)-halfMW
            temp(rr,cc) = sum(sum(confidenceGray(rr-halfsz:rr+halfsz,cc-halfsz:cc+halfsz)));
        end
    end
    [ a b ] = find(temp == max(max(temp)));
    targetLocationT(1) = round(mean(a));
    targetLocationT(2) = round(mean(b));
    drawBoundingBox(targetLocationT(1)-halfsz,targetLocationT(1)+halfsz,...
                    targetLocationT(2)-halfsz,targetLocationT(2)+halfsz,'k');  
